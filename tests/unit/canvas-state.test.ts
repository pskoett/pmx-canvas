...
describe('canvas state manager', () => {
  ...
  test('persists and snapshots canvas-bound AX work items, approvals, and review annotations', async () => {
    ...
    const workItem = canvasState.addWorkItem(
      { title: 'Wire auth', status: 'in-progress', nodeIds: [node.id] },
      { source: 'api' },
    );
  ...
  test('defaults a body-only review annotation to an unanchored note, but rejects an explicit bad node anchor', () => {
    ...
    expect(
      canvasState.addReviewAnnotation(
        { body: 'bad anchor', anchorType: 'node', nodeId: 'does-not-exist' },
        { source: 'api' },
      ),
    ).toBeNull();

    // A file anchor needs no node, and a valid node anchor succeeds.
    const fileReview = canvasState.addReviewAnnotation(
      { body: 'file note', anchorType: 'file', file: 'src/x.ts' },
      { source: 'api' },
    );
    ...
    const nodeReview = canvasState.addReviewAnnotation(
      { body: 'node note', anchorType: 'node', nodeId: anchorNode.id },
      { source: 'api' },
    );
    ...
    expect(
      canvasState
        ...
        .map((r) => r.id)
        .sort(),
    ).toEqual([bodyOnly!.id, fileReview!.id, nodeReview!.id].sort());
  });
  ...
  test('clear() empties canvas-bound AX state but keeps timeline and host capability', async () => {
    ...
    const host = canvasState.setHostCapability(
      { host: 'copilot', capabilities: { canvas: true, sessionMessaging: true } },
      { source: 'api' },
    );
  ...
  test('grouping shifts a packed group clear of existing groups', () => {
    ...
    expect(overlap({ ...groupA.position, ...groupA.size }, { ...groupB.position, ...groupB.size })).toBe(false);
  });
