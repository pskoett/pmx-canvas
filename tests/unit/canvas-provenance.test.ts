...
describe('canvas node provenance', () => {
  test('serializes normalized provenance for source-backed nodes', () => {
    ...
    const fileNode = serializeCanvasNode(
      makeNode({
        id: 'file-1',
        type: 'file',
        data: {
    ...
    const webpageNode = serializeCanvasNode(
      makeNode({
        id: 'web-1',
        type: 'webpage',
        data: {
    ...
    const artifactNode = serializeCanvasNode(
      makeNode({
        id: 'app-1',
        type: 'mcp-app',
        data: {
...
describe('canvas node kind discriminator', () => {
  test('classifies fresh web-artifact nodes via viewerType', () => {
    const fresh = serializeCanvasNode(
      makeNode({
        id: 'wa-fresh',
        type: 'mcp-app',
        data: {
  ...
  test('classifies future URL-only web-artifact nodes via viewerType alone', () => {
    ...
    const urlOnly = serializeCanvasNode(
      makeNode({
        id: 'wa-url',
        type: 'mcp-app',
        data: {
  ...
  test('classifies legacy v0.1.3 web-artifact nodes via hostMode+path fallback', () => {
    ...
    const legacy = serializeCanvasNode(
      makeNode({
        id: 'wa-legacy',
        type: 'mcp-app',
        data: {
  ...
  test('classifies external-app nodes by mode', () => {
    const extApp = serializeCanvasNode(
      makeNode({
        id: 'ea-1',
        type: 'mcp-app',
        data: {
  ...
  test('falls back to "mcp-app" for plain MCP-app nodes', () => {
    const plain = serializeCanvasNode(
      makeNode({
        id: 'mcp-1',
        type: 'mcp-app',
...
describe('agent-safe canvas serialization', () => {
  const shellHtml =
    '<!doctype html><html><body><script type="module">large repeated iframe shell</script></body></html>';
  ...
  test('preserves non-external-app HTML payloads', () => {
    const artifact = serializeCanvasNodeForAgent(
      makeNode({
        id: 'artifact-1',
        type: 'mcp-app',
    ...
    const htmlNode = serializeCanvasNodeForAgent(
      makeNode({
        id: 'html-1',
        type: 'html',
        data: { html: shellHtml },
