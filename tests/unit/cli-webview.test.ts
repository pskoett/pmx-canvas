...
describe('agent CLI webview commands', () => {
  ...
  test('html subcommand routes to agent CLI instead of server startup', async () => {
    const originalArgv = process.argv;
    const originalExit = process.exit;
    const originalLog = console.log;
    const log = mock(() => {});
    ...
    console.log = log;
    process.exit = exitMock as typeof process.exit;
    process.argv = ['bun', 'src/cli/index.ts', 'html', 'primitive', 'schema', '--help'];

    try {
      await import(`../../src/cli/index.ts?html-route-test=${Date.now()}`);
    } finally {
      process.argv = originalArgv;
      process.exit = originalExit;
      console.log = originalLog;
    ...
    expect(exitMock).not.toHaveBeenCalled();
    const output = log.mock.calls.map((call) => String(call[0] ?? '')).join('\n');
    expect(output).toContain('pmx-canvas html primitive schema');
    expect(output).not.toContain('Server options:');
  });

  test('diagram subcommand routes to agent CLI instead of server startup', async () => {
    const originalArgv = process.argv;
    const originalExit = process.exit;
    const originalLog = console.log;
    const log = mock(() => {});
    ...
    console.log = log;
    process.exit = exitMock as typeof process.exit;
    process.argv = ['bun', 'src/cli/index.ts', 'diagram', 'add', '--help'];

    try {
      await import(`../../src/cli/index.ts?diagram-route-test=${Date.now()}`);
    } finally {
      process.argv = originalArgv;
      process.exit = originalExit;
      console.log = originalLog;
    ...
    expect(exitMock).not.toHaveBeenCalled();
    const output = log.mock.calls.map((call) => String(call[0] ?? '')).join('\n');
    expect(output).toContain('pmx-canvas diagram add');
    expect(output).toContain('external-app add --kind excalidraw');
    expect(output).not.toContain('Server options:');
  });

  test('fit subcommand routes to agent CLI instead of server startup', async () => {
    const originalArgv = process.argv;
    const originalExit = process.exit;
    const originalLog = console.log;
    const log = mock(() => {});
    ...
    console.log = log;
    process.exit = exitMock as typeof process.exit;
    process.argv = ['bun', 'src/cli/index.ts', 'fit', '--help'];

    try {
      await import(`../../src/cli/index.ts?fit-route-test=${Date.now()}`);
    } finally {
      process.argv = originalArgv;
      process.exit = originalExit;
      console.log = originalLog;
    ...
    expect(exitMock).not.toHaveBeenCalled();
    const output = log.mock.calls.map((call) => String(call[0] ?? '')).join('\n');
    expect(output).toContain('pmx-canvas fit');
    expect(output).not.toContain('Server options:');
  });
