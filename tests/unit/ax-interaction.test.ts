...
function makeManager(node: CanvasNodeState | undefined) {
  ...
  const manager: AxInteractionManager = {
    ...
    recordAxEvent: (input, o) => { rec('recordAxEvent', input, o); return { id: 'ev1', seq: 1, kind: input.kind, summary: input.summary, detail: input.detail ?? null, nodeIds: input.nodeIds ?? [], data: input.data ?? null, createdAt: 't', source: o?.source ?? null }; },
    recordSteeringMessage: (message, o) => { rec('recordSteeringMessage', message, o); return { id: 's1', seq: 1, message, delivered: false, createdAt: 't', source: o?.source ?? null }; },
    addWorkItem: (input, o) => { rec('addWorkItem', input, o); return { id: 'w1', title: input.title, status: input.status ?? 'todo', detail: input.detail ?? null, nodeIds: input.nodeIds ?? [], createdAt: 't', updatedAt: 't', source: o?.source ?? null }; },
    updateWorkItem: (id, patch, o) => { rec('updateWorkItem', id, patch, o); return id === 'missing' ? null : { id, title: patch.title ?? 'x', status: patch.status ?? 'todo', detail: patch.detail ?? null, nodeIds: patch.nodeIds ?? [], createdAt: 't', updatedAt: 't', source: o?.source ?? null }; },
    addEvidence: (input, o) => { rec('addEvidence', input, o); return { id: 'ev2', seq: 1, kind: input.kind, title: input.title, body: input.body ?? null, ref: input.ref ?? null, nodeIds: input.nodeIds ?? [], data: input.data ?? null, createdAt: 't', source: o?.source ?? null }; },
    requestApproval: (input, o) => { rec('requestApproval', input, o); return { id: 'a1', title: input.title, detail: input.detail ?? null, action: input.action ?? null, status: 'pending', nodeIds: input.nodeIds ?? [], createdAt: 't', resolvedAt: null, resolution: null, source: o?.source ?? null }; },
    resolveApproval: (id, decision, o) => { rec('resolveApproval', id, decision, o); return id === 'missing' ? null : { id, title: 'x', detail: null, action: null, status: decision, nodeIds: [], createdAt: 't', resolvedAt: 't', resolution: o?.resolution ?? null, source: o?.source ?? null }; },
    addReviewAnnotation: (input, o) => { rec('addReviewAnnotation', input, o); return { id: 'r1', kind: input.kind ?? 'comment', body: input.body, severity: input.severity ?? 'info', status: 'open', anchorType: input.anchorType ?? 'node', nodeId: input.nodeId ?? null, file: input.file ?? null, region: input.region ?? null, author: input.author ?? null, createdAt: 't', updatedAt: 't', source: o?.source ?? null }; },
...
describe('resolveNodeAxCapabilities', () => {
  ...
  test('per-node override can opt-in and narrow within the ceiling', () => {
    const caps = resolveNodeAxCapabilities(makeNode('html', { axCapabilities: { enabled: true, allowed: ['ax.work.create'] } }));
    expect(caps.enabled).toBe(true);
  ...
  test('per-node override cannot grant beyond the type ceiling', () => {
    ...
    const caps = resolveNodeAxCapabilities(makeNode('file', { axCapabilities: { enabled: true, allowed: ['ax.steer', 'ax.evidence.add'] } }));
...
describe('normalizeNodeAxCapabilities', () => {
  test('drops unknown interaction types and invalid shapes', () => {
    expect(normalizeNodeAxCapabilities({ enabled: true, allowed: ['ax.work.create', 'nope'] }))
      .toEqual({ enabled: true, allowed: ['ax.work.create'] });
...
describe('applyAxInteraction', () => {
  test('accepts an allowed interaction and maps it to the AX op', () => {
    ...
    const { result, events } = applyAxInteraction(manager, { type: 'ax.work.create', sourceNodeId: 'status-1', payload: { title: 'Wire auth' } }, 'api');
  ...
  test('rejects an interaction the node type cannot emit', () => {
    ...
    const { result } = applyAxInteraction(manager, { type: 'ax.steer', sourceNodeId: 'file-1', payload: { message: 'go' } }, 'api');
  ...
  test('rejects when AX is disabled for the node', () => {
    ...
    const { result } = applyAxInteraction(manager, { type: 'ax.work.create', sourceNodeId: 'html-1', payload: { title: 'x' } }, 'api');
  ...
  test('rejects an unknown source node', () => {
    ...
    const { result } = applyAxInteraction(manager, { type: 'ax.work.create', sourceNodeId: 'ghost', payload: { title: 'x' } }, 'api');
  ...
  test('rejects an invalid payload', () => {
    ...
    const { result } = applyAxInteraction(manager, { type: 'ax.work.create', sourceNodeId: 'status-1', payload: {} }, 'api');
  ...
  test('maps evidence / review / approval / steer to their ops', () => {
    ...
    expect(applyAxInteraction(file.manager, { type: 'ax.evidence.add', sourceNodeId: 'file-1', payload: { kind: 'file', title: 'log' } }, 'api').result.ok).toBe(true);
    expect(file.calls.some((c) => c.op === 'addEvidence')).toBe(true);

    const file2 = makeManager(makeNode('file'));
    const review = applyAxInteraction(file2.manager, { type: 'ax.review.add', sourceNodeId: 'file-1', payload: { body: 'looks off' } }, 'api');
    expect(review.result.ok).toBe(true);
    expect((file2.calls.find((c) => c.op === 'addReviewAnnotation')?.args[0] as { nodeId: string }).nodeId).toBe('file-1');

    const status = makeManager(makeNode('status'));
    expect(applyAxInteraction(status.manager, { type: 'ax.approval.request', sourceNodeId: 'status-1', payload: { title: 'Deploy?' } }, 'api').result.ok).toBe(true);
    expect(status.calls.some((c) => c.op === 'requestApproval')).toBe(true);

    const md = makeManager(makeNode('markdown'));
    expect(applyAxInteraction(md.manager, { type: 'ax.steer', sourceNodeId: 'markdown-1', payload: { message: 'go' } }, 'api').result.ok).toBe(true);
  ...
  test('maps elicitation + mode requests (Phase 5 primitives)', () => {
    ...
    const elic = applyAxInteraction(jr.manager, { type: 'ax.elicitation.request', sourceNodeId: 'json-render-1', payload: { prompt: 'owner?', fields: ['owner'] } }, 'api');
    ...
    const mode = applyAxInteraction(st.manager, { type: 'ax.mode.request', sourceNodeId: 'status-1', payload: { mode: 'execute', reason: 'plan approved' } }, 'api');
    expect(mode.result.ok).toBe(true);
    expect(st.calls.some((c) => c.op === 'requestMode')).toBe(true);

    // mode requires a valid enum value
    const bad = applyAxInteraction(makeManager(makeNode('status')).manager, { type: 'ax.mode.request', sourceNodeId: 'status-1', payload: { mode: 'turbo' } }, 'api');
  ...
  test('sandboxed surfaces cannot target arbitrary nodes (nodeIds clamped to source)', () => {
    const { manager, calls } = makeManager(makeNode('html', { axCapabilities: { enabled: true, allowed: ['ax.focus.set', 'ax.work.create'] } }));
    // html-node surface tries to focus a DIFFERENT node — must be clamped to source.
    const focus = applyAxInteraction(manager, { type: 'ax.focus.set', sourceNodeId: 'html-1', sourceSurface: 'html-node', payload: { nodeIds: ['other-node', 'another'] } }, 'browser');
    expect(focus.result.ok).toBe(true);
    expect(calls.find((c) => c.op === 'setAxFocus')?.args[0]).toEqual(['html-1']);

    const work = applyAxInteraction(manager, { type: 'ax.work.create', sourceNodeId: 'html-1', sourceSurface: 'html-node', payload: { title: 'x', nodeIds: ['victim'] } }, 'browser');
  ...
  test('rejects update of a non-existent work item (404)', () => {
    ...
    expect(applyAxInteraction(wm.manager, { type: 'ax.work.update', sourceNodeId: 'status-1', payload: { id: 'missing', status: 'done' } }, 'api').result)
  ...
  test('opt-in html node can emit an allowed interaction', () => {
    const { manager, calls } = makeManager(makeNode('html', { axCapabilities: { enabled: true, allowed: ['ax.focus.set'] } }));
    const { result } = applyAxInteraction(manager, { type: 'ax.focus.set', sourceNodeId: 'html-1', payload: {} }, 'api');
    expect(result.ok).toBe(true);
    expect(calls.find((c) => c.op === 'setAxFocus')?.args[0]).toEqual(['html-1']);
  });
});
