...
describe('code graph', () => {
  ...
  test('parseImports handles JS/TS, Python, Go, and Rust sources with deduplication', () => {
    ...
    ).toEqual(['./foo', './bar', './side-effect', './lazy', './baz']);

    expect(parseImports('import os\nfrom pkg.module import thing\nimport os\n', '/tmp/example.py')).toEqual([
      'os',
      'pkg.module',
    ]);
    ...
    ).toEqual(['fmt', 'net/http', 'example/internal/app']);

    expect(parseImports('mod parser;\nuse crate::graph::node;\nmod parser;\n', '/tmp/example.rs')).toEqual([
      'parser',
      'graph::node;',
    ]);
  });
  ...
  test('buildCodeGraphSummary and formatCodeGraph describe central and isolated files', () => {
    ...
    canvasState.addNode(
      makeNode({ id: 'app', type: 'file', data: { title: 'App', path: appPath, fileContent: read(appPath) } }),
    );
    canvasState.addNode(
      makeNode({
        id: 'feature',
        type: 'file',
        data: { title: 'Feature', path: featurePath, fileContent: read(featurePath) },
      }),
    );
    canvasState.addNode(
      makeNode({ id: 'util', type: 'file', data: { title: 'Util', path: utilPath, fileContent: read(utilPath) } }),
    );
    canvasState.addNode(
      makeNode({ id: 'lone', type: 'file', data: { title: 'Lone', path: lonePath, fileContent: read(lonePath) } }),
    );
