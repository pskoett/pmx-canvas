...
**Revision, 2026-09-24 (later):** Adds what the promise needs to hold: a one-page current plan, the second half of the promise stated as a bet with a decision rule (Part 1, item 4), a meaningful text form for every node type (move 1), search across boards and library backup (move 0), a smaller 0.7 with export as 0.7.x, and the second-user milestone moved to export.
...
*One page; the rest of this document is the reasoning and the detail. Updated 2026-09-24.*
...
**Decided:** the fleet layer and every node type stay (2026-09-06); boards are the wiki and the destination is "share this board" (2026-09-23); gate answers are human-only with requester withdraw, and the reference surface is Chromium at 600 px for now (2026-09-24). Undecided: generated surfaces (6b).

| Next | Content | Done when |
|---|---|---|
| 0.7 | Many boards stage 1 with the nine real boards restored, library backup, the small trust fixes, read instrumentation, the 600 px reference project | The nine boards open by name; a backup restores them on a clean machine; an agent's approve returns 403; every node type paints at 600 px |
...
4. **Measure the thesis.** Record which resources each agent session reads and whether the pinned nodes were in it, and add one fixed eval under `docs/evals/`: the same task on a curated and an uncurated board. Until this exists, "pins shape the agent's context" is a belief, not a finding. The decision rule, set now so the result cannot be argued away later: if across real sessions agents do not read the brief, or the curated board does not beat the uncurated one on the eval, the vision says so, stops investing in attention ranking and the brief beyond what already ships, and puts that effort into the human side (the wiki, sharing, tours). If they do, the brief becomes the product's headline and the attention heat (the payoff section) moves forward.
...
|---|---|---|
| 0.7 | Many boards stage 1 with the nine real boards restored, and library backup (move 0); the S half of trust (move 7: boot secret, human-only answers with requester withdraw, `res.ok` toasts, redirect host filter); read instrumentation (Part 1, item 4); the Chromium 600 px reference project (5). | The nine boards open by name; a backup restores them on a clean machine; an agent's approve returns 403 and its withdraw returns `withdrawn`; every node type paints at 600 px; agent reads are being recorded. |
