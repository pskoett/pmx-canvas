...
**Status:** In progress — wave 1 landed (7 composites: `canvas_node`, `canvas_render`, `canvas_edge`, `canvas_group`, `canvas_history`, `canvas_view`, `canvas_query`). Remaining: `canvas_snapshot` (name held by the legacy save tool until v0.3), the AX composites (#11–15, need plan-005 item 7), `canvas_app`/`canvas_webview` (need item 8), and the deferred actions `refresh` / `add-primitive` / `remove-annotation` / board validation.
**Date:** 2026-06-12
